...
BITSTREAM="$PROJECT_ROOT/build/qmtech_wukong/gateware/qmtech_wukong.bit"
...
case "$1" in
    ...
    bVid)
        echo "--- Building SoC with video terminal (wukong board) ---"
        python soc.py --with-video-terminal --build
        ;;

    bVidColorBar)
        echo "--- Building SoC with video terminal color bars (wukong board) ---"
        python soc.py --with-video-colorbars --build
        ;;
    bd)
